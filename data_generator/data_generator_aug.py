...
import numpy as np
from data_generator.image_preprocessing import augment, image_loader_aug
from data_generator.labels_preprocessing import label_loader, label_loader_aug
...
class DataGenerator_aug(Sequence):
    ...
    def __data_generation(self, batch):
        ...
        for index, image_id in enumerate(batch):
            image, aspect_ratio = image_loader_aug(image_id, self.dataset_path, self.input_shape)
            parameters_3DMM = label_loader_aug(image_id, self.labels, aspect_ratio)
            X.append(image)

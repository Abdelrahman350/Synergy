...
def label_loader_aug(image_id, labels, aspect_ratio):
    pose_3DMM = np.array(labels[image_id][0:12])
    alpha_exp = np.ravel(np.array(labels[image_id][52:]))
    alpha_shp = np.ravel(np.array(labels[image_id][12:52]))

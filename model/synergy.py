...
class SynergyNet(Model):
      def __init__(self, input_shape, num_classes=62, num_points=68, **kwargs):
            super(SynergyNet, self).__init__(**kwargs, name="SynergyNet")
            self.input_shape_ = input_shape
            self.mobileNet = create_MobileNetV2(input_shape=input_shape, classes=num_classes)
            self.GlobalAvgBooling = GlobalAveragePooling2D(name='Global_Avg_Pooling')
            ...
            self.morphable_model = Reconstruct_Vertex(name='Morphable_layer')
      ...
      def model(self):
            images = Input(shape=self.input_shape_, name='Input_Images')
            return Model(inputs=[images], outputs=self.call(images), name='SynergyNet')
      
      def summary(self):
            return self.model().summary()

      def get_config(self):
            base_config = super(SynergyNet, self).get_config()
            return {**base_config, 
                        'input_shape': self.input_shape_,
                        'backbone': self.mobileNet,
                        'flatten': self.flatten,

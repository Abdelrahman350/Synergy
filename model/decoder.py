...
def Landmarks_to_3DMM(num_classes=62, num_points=68):
    ...
    global_features = MaxPool1D(pool_size=num_classes, name='MaxPool1D')(relu5)
    ...
    pose_3DMM = ReLU(name='ReLU_6')(bn6)

    # Regressing expression parameters
    conv7 = Conv1D(filters=10, kernel_size=1, name='Conv1D_7')(global_features)
    bn7 = BatchNormalization(name='BatchNormalization_7')(conv7)
    alpha_exp = ReLU(name='ReLU_7')(bn7)

    # Regressing shape parameters
    conv8 = Conv1D(filters=40, kernel_size=1, name='Conv1D_8')(global_features)
    bn8 = BatchNormalization(name='BatchNormalization_8')(conv8)
    alpha_shp = ReLU(name='ReLU_8')(bn8)
